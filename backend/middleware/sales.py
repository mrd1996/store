...
def main():
    ...
    response = requests.get(
        f"https://store.steampowered.com/search/?specials=1&page=1")
    ...
    for p in range(2, int(last[last.find("page")+5:])):
        ...
        for x in anchors:
            ...
            if (div.strike):
                ...
                if (div.find("br")):
                    sale[str(x['data-ds-appid'])
                         ]['price'] = ''.join(div.find("br").next_siblings).strip()
                sale[str(x['data-ds-appid'])]['oldprice'] = div.strike.text
                sale[str(x['data-ds-appid'])
                     ]['discount'] = div.find(noStyle).span.text
                sales.append(sale)
        response = requests.get(
            f"https://store.steampowered.com/search/?specials=1&page={p}")
        soup = BeautifulSoup(response.content, "lxml")
